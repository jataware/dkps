...
axL.text(0.30, 0.185, f"IRT off-scale ({irt.loc[1.0]:.2f}$\\to${irt.loc[0.0]:.2f})",
         ha='center', va='top', fontsize=11.9, color=STYLE['irt']['color'], fontweight='bold')
axL.set_xlabel(r"fraction of paired queries  $\rho = m_{ii'}/M_{ij}$", fontsize=15.6)
axL.set_ylabel('MAE vs. full-eval score', fontsize=15.6)
axL.set_title('(a) the cliff: PKPS holds as queries unpair', fontsize=15.6)
axL.legend(frameon=False, fontsize=12.5, ncol=2, loc='upper right')
...
axR.text(0.04 * lim, 0.96 * lim, 'PKPS better', color=STYLE['pkps']['color'], fontsize=13.1,
         va='top', fontweight='bold')
axR.set_xlim(0, lim); axR.set_ylim(0, lim); axR.set_aspect('equal')
axR.set_xlabel('PKPS MAE (per model)', fontsize=15.6); axR.set_ylabel('DKPS MAE (per model)', fontsize=15.6)
axR.set_title(f'(b) per model: PKPS $<$ DKPS for {frac:.0%}', fontsize=15.6)
axR.tick_params(labelsize=13.1)

fig.suptitle('Unpaired evaluation', fontsize=17.5, fontweight='bold', y=1.0)

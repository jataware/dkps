...
# def bayes_shrink_mean(score_matrix: np.ndarray, new_scores: np.ndarray) -> float:
#     model_means = score_matrix.mean(axis=0)
#     mu0         = model_means.mean()
    
#     var_within  = score_matrix.var(ddof=1, axis=0).mean()
#     var_between = model_means.var(ddof=1)
#     k = var_within / var_between
    
#     n_new = new_scores.shape[0]
#     x_bar = new_scores.mean()
    
#     pred = (k * mu0 + n_new * x_bar) / (k + n_new)
#     return float(pred)

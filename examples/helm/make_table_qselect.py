...
The "best" query set is selected as the seed with highest average R²
across all models for each (dataset_split, n_samples).
"""
...
for tsv_path in tqdm(tsv_paths, desc='Loading results'):
    try:
        ...
        print(f'Error loading {tsv_path}')
        continue
...
tab_split['e_dkps_best_vs_avg'] = tab_split['e_lr_dkps'] - tab_split['e_lr_dkps_best']
tab_split['e_interp_best_vs_avg'] = tab_split['e_interp'] - tab_split['e_interp_best']
...
tab_split = tab_split.rename(columns={
    ...
    'e_dkps_best_vs_avg': 'DKPS Δ(avg-best)',
    'e_interp_best_vs_avg': 'Interp Δ(avg-best)',
})

tab_split = tab_split.reset_index()
print(tab_split)
outpath_split = TABLES_DIR / f'table_qselect-v2-{EMBED_DIR}-n_models={args.n_models}-by_dataset_split.tsv'
...
if 'e_lr_dkps_best' in tab_dataset.columns:
    tab_dataset['e_dkps_best_vs_avg'] = tab_dataset['e_lr_dkps'] - tab_dataset['e_lr_dkps_best']
    tab_dataset['e_interp_best_vs_avg'] = tab_dataset['e_interp'] - tab_dataset['e_interp_best']
...
tab_dataset = tab_dataset[~((tab_dataset.index.get_level_values('dataset') == 'math') & (tab_dataset.index.get_level_values('n_samples') == 64))]

tab_dataset = tab_dataset.reset_index()
print(tab_dataset)
outpath_dataset = TABLES_DIR / f'table_qselect-v2-{EMBED_DIR}-n_models={args.n_models}-by_dataset.tsv'
